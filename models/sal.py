...
class MultiModalEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None):
        ...
        self.semantic_ocr_embedding = SemanticOCREmbedding(config, embed_tokens)
        self.semantic_object_embedding = SemanticObjectEmbedding(config, embed_tokens)

        self.text_sparate_embedding = TextSemanticSeparate(config)
    ...
    def forward(
        ...
        # embedding question features
        inputs_embeds = self.embed_tokens(input_ids)
        
        # multimodal embeddings
        inputs_embeds = torch.cat([inputs_embeds, ocr_features, object_features], dim=1)
        ...
        if self.is_decoder and encoder_attention_mask is None and encoder_hidden_states is not None:
            ...
            encoder_attention_mask = torch.ones(
                B, encoder_seq_length, device=inputs_embeds.device, dtype=torch.long
            )
