...
class OcrDataset(FeatureDataset):
    def __init__(self, json_path: str, vocab, config) -> None:
        ...
        self.image_feature_path = config.feature_path.features
    ...
    def __getitem__(self, idx: int):
        ...
        shifted_right_answer_tokens = torch.zeros_like(answer_tokens).fill_(self.vocab.padding_token_idx)
        shifted_right_answer_tokens[:-1] = answer_tokens[1:]
        answer_tokens = torch.where(answer_tokens == self.vocab.eos_token_idx, self.vocab.padding_token_idx, answer_tokens) # remove eos_token in answer
